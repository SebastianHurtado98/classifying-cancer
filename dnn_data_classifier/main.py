...
def construct_net(num_features, model_dir, n):
    """Constructs a 3 layer Deep Neural Net with 10, 20, 10 units"""

    """Updated: N number of layers for comparison"""
    ...
    unit = 10
    for i in range(int(number_of_layers/2)):
        layers.append(unit)
        unit = unit + 10
    unit = unit - 10
    for i in range(int(number_of_layers/2)):
...
if __name__ == '__main__':
    ...
    print("train data : ", train_set.data)
    print("train target : ", train_set.target)
    print("test: ", test_set[0])

    # Added for comparison with other models
    class_metric_full_process(train_set.data, train_set.target)

    dnn_model = construct_net(num_features=9, model_dir=model_dir, n=16)
